...
public class PrivateBankAlt implements Bank{
     ...
     */
    public PrivateBankAlt(String p_name, double p_incomingInterest, double p_outgoingInterest) {
        this.setName(p_name);
     ...
     */
    public PrivateBankAlt(PrivateBankAlt p_privateBank) {
    ...
    public boolean equals(Object p_object) {
        ...
        if(!(p_object instanceof PrivateBankAlt)) {
            return false;
        }
        //cast and compare
        PrivateBankAlt object = (PrivateBankAlt) p_object;
    ...
    public void addTransaction(String p_account, Transaction p_transaction) throws TransactionAlreadyExistException, AccountDoesNotExistException, TransactionAttributeException, TransactionAttributeValidationException {
        //direct reference to Payment unavoidable
    ...
    public double getAccountBalance(String p_account) {
        ...
        if (accountTransactions != null) {
            for (Transaction transaction : accountTransactions) {
                ...
                //large discrepancy with the normal version, since Transfer objects usually don't differentiate between outgoing and incoming
                //i would need to rewrite calculate or also add abstract keyword to Transfer
                else if(((Transfer)transaction).getSender().equals(p_account)) {
                    balance -= transaction.calculate();
                }
                else {
                    balance += transaction.calculate();
    ...
    public List<Transaction> getTransactionsSorted(String p_account, boolean p_asc) {
        ...
        return accountTransactions != null ? new ArrayList<>(accountTransactions) : new ArrayList<>();
    }
